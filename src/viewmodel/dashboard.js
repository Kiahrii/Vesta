...
export function useDashboardMetrics() {
  ...
  const fetchDashboardMetrics = useCallback(async () => {
    const [roomsResult, tenantsResult, reportsResult, paymentsResult] = await Promise.all([
      supabaseRead.from('rooms').select('room_id, monthly_rent'),
      supabaseRead.from('tenants').select('tenant_id'),
      supabaseRead.from('maintenance_report').select('status'),
      supabaseRead.from('payments').select('status, amount_paid')
    ]);
  ...
  } = useSupabaseQuery(fetchDashboardMetrics, {
    initialData: INITIAL_METRICS,
    deps: [fetchDashboardMetrics],
